---
title: "Three quarters of the tasks workers grab with AI are gone a month later"
...
The second half is the study's real finding: 23.6% is not low. Matched workers who were otherwise comparable but had no observed use of a given task in the prior month picked it up the next month only 8.4% of the time. One observed use nearly triples the odds of next-month use. And the part that sticks accumulates: among roughly 6,200 workers observed continuously from April to July, previously used cross-occupation tasks grew from 13.1% of their occupation-specific AI activity in April to 25.9% in July, nearly doubling in four months.

No job description changed a word. Yet a quarter of these workers' occupation-specific AI use now consists of tasks taken from other jobs that they had already used before. That number deserves a longer pause than the headline one.
...
At the bottom sits "explaining financial information", at 15%.

Set against the July report's entry-point data, this list is a little surprising. Back then, the most commonly attempted cross-occupation tasks were financial calculations, technical troubleshooting, and creating marketing materials, and the first two ranked among the top three in all seven other occupation groups. The busiest entry points were finance and tech tasks; the top of the persistence list skews toward marketing and customer conversation. To be clear, this is not a before-and-after of the same attempts: entry volume and next-month persistence are two different metrics from two different reports. I'm only comparing the shape of the two lists.

"AI unlocks new skills" usually conjures marketers learning to run data analysis or HR learning to write scripts. The data's answer is more mundane: what most reliably becomes routine is people in every job starting to do their own selling.
...
The first is the calendar. Marketing output is periodic by nature: social posts go out every week, promotional emails every cycle, customer conversations about the product every day. Explaining financial information is an occasional need; once you have made sense of a quarterly report or compared two options, you're done. And a next-month return rate is structurally kind to high-frequency tasks: it only checks whether a task reappears in the month right after it was first observed. A task needed twice a year would score as "didn't stick" even if AI handled it perfectly every time. So this ranking measures a blend of where AI is useful and how dense the task is on the calendar, and the report doesn't separate the two.
