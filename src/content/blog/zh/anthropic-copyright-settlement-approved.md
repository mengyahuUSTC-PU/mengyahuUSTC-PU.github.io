...
**第一步是拿书，合法与否取决于书的来路。** Anthropic 拿书走了两条路：一条是从那两个盗版库直接下载 700 多万本电子书，存成一个内部的「中央图书馆」；另一条是后来花钱买实体书，拆开扫描成电子版、销毁原书。Alsup 判定：买书再扫描属于合理使用——自己花钱买的书，换个格式存起来备用，没问题；违法的是盗版下载——问题不在「建了个库」，而在这个库里的书是偷来的。更糟的是，库里还有大量 Anthropic 自己都决定不用于训练的盗版书，一直原样存着，连「为训练服务」这层辩护都套不上（[Reuters](https://wsau.com/2026/07/20/us-judge-approves-anthropics-1-5-billion-settlement-of-copyright-lawsuit/)）。所以并不存在「存书不行、训练就行」的悖论：为训练而存合法买来的书，本身就是被法院认可的。
...
从机制上看，和解几乎是必然的均衡。但要看懂为什么，得先说清「和解」和「败诉判例」的区别——区别不在赔不赔钱，而在赔多少、按谁的规矩赔、以后还能不能接着干。
...
败诉判例则会把这个筹码没收。假设本案不和解、一路打完上诉，第九巡回法院推翻「训练=合理使用」，那么从判决之日起，辖区内所有法官都必须照此裁判。下一批作者起诉任何一家 AI 公司时，「侵权是否成立」这一仗已经有人替他们打赢了，直接进入算账环节：按法定区间（故意侵权每部最高 15 万美元）要价，没有折扣可谈。所以「归根结底不还是赔钱」并不成立——没判例时，赔的是打折价，一案一结；有判例后，赔的是对方开的全价，而且每个后来的原告手里都攥着同一张胜诉票。

至于「灭顶之灾」，说的甚至不是钱。若上诉法院认定训练侵权，Anthropic 已发布的模型就全部建立在侵权行为之上，法院可以颁发禁令，责令停用这些模型；此后每一次训练都要先逐一拿到授权。赔款是一次性的支出，禁令终结的是「用书训练模型」这条路本身。这就是风险收益不对称的全部含义：上诉赢了，省下的只是和解金；上诉输了，可能输掉整个商业模式。理性选择自然是花钱买确定性。

于是每个案子都倾向于在形成上诉判例之前和解掉。反复被诉、反复和解，确实要反复付钱，但每次付的都是带不确定性折扣的价；而一场败诉，付的是全价，外加禁令，外加把胜诉票发给所有后来的原告。行业一次次付钱，把规则一次次买出法庭，算的就是这笔账。
...
- **它不是全行业通行证。** 这是地区法院一审裁定，只约束本案——上一节说的「对其他法官只有说服力、没有约束力」在这里同样适用。别家 AI 公司照做，被诉时仍要重新说服自己的主审法官接受同样的分析。
