...
**硬的只有那一句承诺**——公开追踪、公开报告。我去看了承接它的网站：[claude.com/hard-questions](https://claude.com/hard-questions) 目前更像品牌页，我抓取时只看到一条展示中的问题（「AI 能否帮助更快的医学诊断？」）和一个「分享你的希望」入口。页面上「查看目前进展」的链接指向 Anthropic 官网的进度页《[Our work on the hard questions about AI](https://www.anthropic.com/path-to-hope)》。点开后是一个云图，云图上的每个点是一张图片，点开图片就是一篇对应的文章，讲 Anthropic 在某个问题上做了什么。内容不算少，但形态是文章集：一篇篇讲成果的故事，而不是一份带日期、能逐条核对进度、包含「这件没做到」的行动清单。
...
自我报告机制的通病都在这里：指标自定，口径自定，不利的项可以沉默。博文承诺过「达不成也透明」，这句写下来是要被将来对照的，但对照的前提是追踪页真的长成一份带时间戳、可逐条核对的行动清单，而目前它的形态是文章集，不是清单。
...
第二，追踪页是否长出可核对的内容。判断标准很朴素：有没有具体行动条目、有没有日期、有没有「这件没做到」的记录。一面精选故事墙不算数，而目前的进度页正是这个形态。
...
我的综合判断：这不是一篇空洞的公关稿，数据部分经得起核查，承诺部分写法上留了被追责的接口。但截至今天，可核查的只有民调；追踪页上挂的是展示成果的文章，还不是能记分的记分牌。故事墙不记分。
...
- [Our work on the hard questions about AI — Anthropic](https://www.anthropic.com/path-to-hope) — 「查看目前进展」指向的进度页：云图形式的文章集
...
<!-- 核查说明：path-to-hope 页面标题《Our work on the hard questions about AI》已用抓取工具核实；云图及「每个点是一篇文章」的形态为作者亲见描述，抓取工具因页面为 JS 渲染只能取到导航结构，未能逐篇枚举文章列表。「形态是文章集而非带日期的行动清单」基于页面现状判断，若后续页面改版增加日期/进度条目，第二个检验信号的表述需更新。 -->
