...
Bruce Schneier 和南加大的 Barath Raghavan 在 7 月 24 日发表的文章里把这个空洞点了出来，并给出一个提案：给 AI 算一个「精灵系数」（Genie Coefficient），量化系统实际执行结果与用户真实意图之间的偏差（[原文](https://www.schneier.com/blog/archives/2026/07/why-ai-needs-a-genie-coefficient.html)，首发于 IEEE Spectrum）。名字取自民间故事里的许愿精灵：愿望会被兑现，但兑现的方式经常让许愿人后悔。
...
Schneier 和 Raghavan 提出的测量基准是法律里的老概念——**合理人标准**：不问系统怎么解释、也不问用户内心怎么想,而是问「一个通情达理的普通人会把这句指令理解成什么意思，系统做的是不是这件事」。上面 OpenAI 严重度三级的定义（「合理用户不会预料到且会强烈反对」）几乎就是同一句话——厂商内部其实已经在算一个私有版的精灵系数了。

他们对测试设计给了几条具体原则：题目里要埋好诱人的捷径和字面歧义（专门钓那两类精灵行为）；用真实工具在隔离副本环境里跑，不用玩具环境；同一任务在不同自主度下测，找出「给到哪一档权限就开始出事」的边界；失败按潜在危害加权，删库和排版错误不能同分；最后，报最坏情况而不是平均分——对一个拿着 shell 权限的 agent，用户关心的从来是下限。
...
再看量化本身的难度。合理人标准需要人类判断，这意味着贵、慢、且有争议——不同评审对「合理理解」的分歧不会小。更难的是测最坏情况：OpenAI 自己说严重度三级的绝对发生率很低，而低频高危事件恰恰是采样最难测准的东西，测试量不够，最坏情况根本采不到。所以我的判断是：精灵系数短期内成不了一个可靠的「数」。
